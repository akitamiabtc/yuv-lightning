...
impl Writeable for Route {
	fn write<W: crate::util::ser::Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		write_tlv_fields!(writer, {
			...
			(200, self.route_params.as_ref().and_then(|p| p.yuv_pixel.as_ref()), option),
		});
		Ok(())
	}
}
	...
	fn effective_capacity(&self) -> EffectiveCapacity {
		match self {
			CandidateRouteHop::FirstHop(hop) => EffectiveCapacity::ExactLiquidity {
				...
				liquidity_yuv: hop.details.yuv_holder_pixel.map(|pixel| pixel.luma.amount),
			},
			CandidateRouteHop::PublicHop(hop) => hop.info.effective_capacity(),
			CandidateRouteHop::PrivateHop(PrivateHopCandidate { hint: RouteHintHop { htlc_maximum_msat: Some(max), htlc_maximum_yuv, .. }, .. }) =>
			...
			CandidateRouteHop::Blinded(hop) =>
				EffectiveCapacity::HintMaxHTLC { amount_msat: hop.hint.0.htlc_maximum_msat, amount_yuv: None, },
...
where L::Target: Logger {
	...
	macro_rules! add_entries_to_cheapest_to_target_node {
		( $node: expr, $node_id: expr, $next_hops_value_contribution: expr,
		  $next_hops_cltv_delta: expr, $next_hops_path_length: expr, $next_hops_value_yuv_contribution: expr) => {
	...
	'paths_collection: loop {
		...
		payee_node_id_opt.map(|payee| first_hop_targets.get(&payee).map(|first_channels| {
			for details in first_channels {
				...
				let (contribution_msat, contribution_yuv) = add_entry!(&candidate, 0, path_value_msat,
									0, 0u64, 0, 0, path_value_yuv);
		...
		payee_node_id_opt.map(|payee| match network_nodes.get(&payee) {
			...
			None => {},
			Some(node) => {
				add_entries_to_cheapest_to_target_node!(node, payee, path_value_msat, 0, 0, path_value_yuv);
			},
		});
		...
		for (hint_idx, hint) in payment_params.payee.blinded_route_hints().iter().enumerate() {
			...
			if let (Some(hop_used_msat), _hop_used_yuv) = add_entry!(&candidate,
			...
			.filter(|route| !route.0.is_empty())
		{
			...
			if first_hop_src_is_reachable {
				...
				for (idx, (hop, prev_hop_id)) in hop_iter.zip(prev_hop_iter).enumerate() {
					...
					let (hop_used_msat_opt, hop_used_yuv_opt) = add_entry!(&candidate,
