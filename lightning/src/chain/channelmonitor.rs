...
use crate::chain::chaininterface::{BroadcasterInterface, FeeEstimator, LowerBoundedFeeEstimator, YuvBroadcaster};
...
use crate::sign::{ChannelDerivationParameters, HTLCDescriptor, SpendableOutputDescriptor, StaticPaymentOutputDescriptor, DelayedPaymentOutputDescriptor, ecdsa::WriteableEcdsaChannelSigner, SignerProvider, EntropySource,
				  DelayedYuvPaymentOutputDescriptor, StaticYuvPaymentOutputDescriptor, YuvStaticOutputDescriptor};
use crate::chain::onchaintx::{ClaimEvent, FeerateStrategy, OnchainTxHandler};
use crate::chain::package::{CounterpartyOfferedHTLCOutput, CounterpartyReceivedHTLCOutput, HolderFundingOutput, HolderHTLCOutput, PackageSolvingData, PackageTemplate, RevokedOutput, RevokedHTLCOutput, get_yuv_htlc_pixel_proof};
...
pub struct HTLCUpdate {
	...
	pub(crate) htlc_value_yuv: Option<u128>,
}
...
impl OnchainEventEntry {
	fn confirmation_threshold(&self) -> u32 {
		...
		match self.event {
			...
			OnchainEvent::MaturingOutput {
				descriptor: SpendableOutputDescriptor::DelayedYuvPaymentOutput(ref descriptor)
			} => {
				// A CSV'd transaction is confirmable in block (input height) + CSV delay, which means
				// it's broadcastable when we see the previous block.
				conf_threshold = cmp::max(conf_threshold, self.height + descriptor.inner.to_self_delay as u32 - 1);
			},
...
impl_writeable_tlv_based_enum_upgradable!(ChannelMonitorUpdateStep,
	...
	(1, LatestCounterpartyCommitmentTXInfo) => {
		...
		(200, to_countersignatory_yuv_pixel, option),
...
#[derive(Clone, Debug)]
struct BroadcastedHolderClaims {
	pub claim_requests: Vec<PackageTemplate>,
	pub revokable_script: Option<(ScriptBuf, PublicKey, RevocationKey)>,
	pub htlc_yuv_revokable_scripts: Vec<ScriptBuf>,
}
...
pub(crate) struct ChannelMonitorImpl<Signer: WriteableEcdsaChannelSigner> {
	...
	/// descriptor.
	broadcasted_htlc_yuv_revokable_scripts: Vec<ScriptBuf>,
...
impl<Signer: WriteableEcdsaChannelSigner> Writeable for ChannelMonitorImpl<Signer> {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), Error> {
		...
		// NOTE: YUV pixel amounts of HTLCs in tx outputs that can be claimed moved from
		// bytes parsing to TLV for back compatibility.
		let mut counterparty_claimable_outpoints_yuv_amounts: HashMap<Txid, Vec<Option<u128>>> = new_hash_map();
		...
		for (ref txid, ref htlc_infos) in self.counterparty_claimable_outpoints.iter() {
			...
			let mut htlc_yuv_amounts = Vec::new();
			for (_, (ref htlc_output, ref htlc_source)) in htlc_infos.iter().enumerate() {
				...
				if let Some(yuv_amount) = htlc_output.yuv_amount {
					htlc_yuv_amounts.push(Some(yuv_amount));
					continue
				}

				htlc_yuv_amounts.push(None);
			}

			counterparty_claimable_outpoints_yuv_amounts.insert(**txid, htlc_yuv_amounts);
		}
		...
		let pending_yuv_payments = Some(self.pending_yuv_payments.clone());
		let counterparty_claimable_outpoints_yuv_amounts = Some(counterparty_claimable_outpoints_yuv_amounts);
		...
		write_tlv_fields!(writer, {
			...
			(200 , self.funding_yuv_pixel, option),
			(201 , pending_yuv_payments, option),
			(202 , self.counterparty_per_commitment_luma, option),
			(203 , counterparty_claimable_outpoints_yuv_amounts, option),
			(204 , self.broadcasted_htlc_yuv_revokable_scripts, optional_vec),
		});
...
impl<Signer: WriteableEcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub(crate) fn new(secp_ctx: Secp256k1<secp256k1::All>, keys: Signer, shutdown_script: Option<ScriptBuf>,
	                  ...
	                  best_block: BestBlock, counterparty_node_id: PublicKey,
					  funding_yuv_pixel: Option<Pixel>, holder_yuv_pixel: Option<Pixel>,
					  channel_id: ChannelId,
		...
		let mut counterparty_per_commitment_luma = None;
		if funding_yuv_pixel.is_some() {
			counterparty_per_commitment_luma = Some(new_hash_map());
		};
		...
		let onchain_tx_handler = OnchainTxHandler::new(
			...
			channel_parameters.clone(), initial_holder_commitment_tx, funding_yuv_pixel, secp_ctx
		);
		...
		Self::from_impl(ChannelMonitorImpl {
			...
			funding_yuv_pixel,
			pending_yuv_payments: new_hash_map(),
	...
	pub(crate) fn provide_initial_counterparty_commitment_tx<L: Deref>(
		...
		to_broadcaster_value_sat: u64, to_countersignatory_value_sat: u64,
		counterparty_yuv_pixel: Option<Pixel>, logger: &L,
	)
	...
	fn provide_latest_counterparty_commitment_tx<L: Deref>(
		...
		commitment_number: u64,
		their_per_commitment_point: PublicKey,
		counterparty_yuv_pixel: Option<Pixel>,
		logger: &L,
	) where L::Target: Logger {
		let mut inner = self.inner.lock().unwrap();
		let logger = WithChannelMonitor::from_impl(logger, &*inner);
		inner.provide_latest_counterparty_commitment_tx(
			txid, htlc_outputs, commitment_number, their_per_commitment_point, counterparty_yuv_pixel, &logger)
	}
	...
	pub(crate) fn provide_payment_preimage<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self,
		payment_hash: &PaymentHash,
		payment_preimage: &PaymentPreimage,
		broadcaster: &B,
		...
		L::Target: Logger,
	{
		...
		inner.provide_payment_preimage(
			payment_hash, payment_preimage, broadcaster, yuv_broadcaster, fee_estimator, &logger)
	}
	...
	pub fn update_monitor<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self,
		...
		L::Target: Logger,
	{
		let mut inner = self.inner.lock().unwrap();
		let logger = WithChannelMonitor::from_impl(logger, &*inner);
		inner.update_monitor(updates, broadcaster, yuv_broadcaster, fee_estimator, &logger)
	}
	...
	/// to you.
	pub fn broadcast_latest_holder_commitment_txn<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self, broadcaster: &B, yuv_broadcaster: Option<YB>, fee_estimator: &F, logger: &L
	)
	where
		B::Target: BroadcasterInterface,
		YB::Target: YuvBroadcaster,
		...
		L::Target: Logger
	{
		...
		let logger = WithChannelMonitor::from_impl(logger, &*inner);
		inner.queue_latest_holder_commitment_txn_for_broadcast(broadcaster, yuv_broadcaster, &fee_estimator, &logger);
	}
	...
	pub fn block_connected<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self,
		header: &Header,
		...
		L::Target: Logger,
	{
		...
		inner.block_connected(
			header, txdata, height, broadcaster, yuv_broadcaster, fee_estimator, &logger)
	}

	/// Determines if the disconnected block contained any transactions of interest and updates
	/// appropriately.
	pub fn block_disconnected<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self,
		header: &Header,
		height: u32,
		broadcaster: B,
		...
		L::Target: Logger,
	{
		...
		inner.block_disconnected(
			header, height, broadcaster, yuv_broadcaster, fee_estimator, &logger)
	}
	...
	pub fn transactions_confirmed<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self,
		header: &Header,
		txdata: &TransactionData,
		height: u32,
		...
		L::Target: Logger,
	{
		...
		inner.transactions_confirmed(
			header, txdata, height, broadcaster, yuv_broadcaster, &bounded_fee_estimator, &logger)
	}
	...
	pub fn transaction_unconfirmed<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self,
		txid: &Txid,
		broadcaster: B,
		yuv_broadcaster: Option<YB>,
		...
		L::Target: Logger,
	{
		...
		inner.transaction_unconfirmed(
			txid, broadcaster, yuv_broadcaster, &bounded_fee_estimator, &logger
		);
	}
	...
	///
	/// [`block_connected`]: Self::block_connected
	pub fn best_block_updated<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self,
		header: &Header,
		height: u32,
		broadcaster: B,
		...
		L::Target: Logger,
	{
		...
		inner.best_block_updated(
			header, height, broadcaster, yuv_broadcaster, &bounded_fee_estimator, &logger
		)
	}
	...
	pub fn rebroadcast_pending_claims<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self, broadcaster: B, yuv_broadcaster: Option<YB>, fee_estimator: F, logger: &L,
	)
	where
		B::Target: BroadcasterInterface,
		...
		L::Target: Logger,
	{
		...
		inner.onchain_tx_handler.rebroadcast_pending_claims(
			current_height, FeerateStrategy::HighestOfPreviousOrNew, &broadcaster, yuv_broadcaster, &fee_estimator, &logger,
		);
	}

	/// Triggers rebroadcasts of pending claims from a force-closed channel after a transaction
	/// signature generation failure.
	pub fn signer_unblocked<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&self, broadcaster: B, yuv_broadcaster: Option<YB>, fee_estimator: F, logger: &L,
	)
	where
		B::Target: BroadcasterInterface,
		...
		L::Target: Logger,
	{
		...
		let current_height = inner.best_block.height;
		inner.onchain_tx_handler.rebroadcast_pending_claims(
			current_height, FeerateStrategy::RetryPrevious, &broadcaster, yuv_broadcaster, &fee_estimator, &logger,
		);
	}
	...
	pub fn is_pending_yuv_tx(&self, txid: &Txid) -> bool {
		self.inner.lock().unwrap().pending_yuv_payments.contains_key(txid)
	}
	...
	pub fn yuv_transaction_confirmed(&self, yuv_tx: YuvTransaction) {
		...
		let channel_id = inner.channel_id.clone();
						...
						if let &Some(ref source) = source_option {
							...
							let entry = OnchainEventEntry {
								...
								event: OnchainEvent::HTLCUpdate {
									...
									htlc_value_yuv: htlc.yuv_amount,
...
impl<Signer: WriteableEcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn provide_initial_counterparty_commitment_tx<L: Deref>(
		...
		to_broadcaster_value: u64, to_countersignatory_value: u64, counterparty_yuv_pixel: Option<Pixel>, logger: &WithChannelMonitor<L>,
	) where L::Target: Logger {
		...
		self.provide_latest_counterparty_commitment_tx(txid, htlc_outputs, commitment_number,
				their_per_commitment_point, counterparty_yuv_pixel, logger);
	}

	fn provide_latest_counterparty_commitment_tx<L: Deref>(
		&mut self, txid: Txid,
		htlc_outputs: Vec<(HTLCOutputInCommitment, Option<Box<HTLCSource>>)>,
		commitment_number: u64, their_per_commitment_point: PublicKey, counterparty_yuv_pixel: Option<Pixel>, logger: &WithChannelMonitor<L>,
	) where L::Target: Logger {
		...
		counterparty_yuv_pixel.map(|yuv_pixel| {
			self.counterparty_per_commitment_luma
				.as_mut()
				.unwrap()
				.insert(txid, yuv_pixel.luma);
		});
	}
	...
	fn provide_latest_holder_commitment_tx(&mut self, holder_commitment_tx: HolderCommitmentTransaction, mut htlc_outputs: Vec<(HTLCOutputInCommitment, Option<Signature>, Option<HTLCSource>)>, claimed_htlcs: &[(SentHTLCId, PaymentPreimage)], nondust_htlc_sources: Vec<HTLCSource>) -> Result<(), &'static str> {
		...
		let mut new_holder_commitment_tx = HolderSignedTx {
			...
			feerate_per_kw: trusted_tx.feerate_per_kw(),
			yuv_pixel: trusted_tx.to_broadcasters_yuv_pixel().cloned(),
		};
	...
	fn provide_payment_preimage<B: Deref, YB: Deref, F: Deref, L: Deref>(
		    ...
		    L::Target: Logger,
	{
		...
		macro_rules! claim_htlcs {
			($commitment_number: expr, $txid: expr) => {
				...
				self.onchain_tx_handler.update_claims_view_from_requests(htlc_claim_reqs, self.best_block.height, self.best_block.height, broadcaster, yuv_broadcaster, fee_estimator, logger);
			}
		}
		...
		if self.broadcasted_holder_revokable_script.is_some() {
			...
			if let Some(holder_commitment_tx) = holder_commitment_tx {
				...
				// transactions.
				let broadcaster_holder_claims = self.get_broadcasted_holder_claims(&holder_commitment_tx, self.best_block.height);
				self.onchain_tx_handler.update_claims_view_from_requests(broadcaster_holder_claims.claim_requests, self.best_block.height, self.best_block.height, broadcaster, yuv_broadcaster, fee_estimator, logger);
			}
		}
	}
	...
	fn generate_claimable_outpoints_and_watch_outputs(&mut self, reason: ClosureReason) -> (Vec<PackageTemplate>, Vec<TransactionOutputs>) {
		...
		if !self.onchain_tx_handler.channel_type_features().supports_anchors_zero_fee_htlc_tx() {
			...
			let mut broadcasted_holder_claims = self.get_broadcasted_holder_claims(
			...
			claimable_outpoints.append(&mut broadcasted_holder_claims.claim_requests);
		}
		(claimable_outpoints, watch_outputs)
	}

	pub(crate) fn queue_latest_holder_commitment_txn_for_broadcast<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&mut self, broadcaster: &B, yuv_broadcaster_opt: Option<YB>, fee_estimator: &LowerBoundedFeeEstimator<F>, logger: &WithChannelMonitor<L>
	)
	where
		B::Target: BroadcasterInterface,
		YB::Target: YuvBroadcaster,
		...
		L::Target: Logger,
	{
		...
		self.onchain_tx_handler.update_claims_view_from_requests(
			claimable_outpoints, self.best_block.height, self.best_block.height,  broadcaster, yuv_broadcaster_opt,
			fee_estimator, logger
		);
	}

	fn update_monitor<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&mut self, updates: &ChannelMonitorUpdate, broadcaster: &B, yuv_broadcaster: Option<YB>, fee_estimator: &F, logger: &WithChannelMonitor<L>
	) -> Result<(), ()>
	where B::Target: BroadcasterInterface,
		YB::Target: YuvBroadcaster,
		F::Target: FeeEstimator,
		L::Target: Logger,
	{
		...
		for update in updates.updates.iter() {
			match update {
				...
				ChannelMonitorUpdateStep::LatestCounterpartyCommitmentTXInfo { commitment_txid, htlc_outputs, commitment_number, their_per_commitment_point, to_broadcaster_yuv_pixel, .. } => {
					log_trace!(logger, "Updating ChannelMonitor with latest counterparty commitment transaction info");
					self.provide_latest_counterparty_commitment_tx(*commitment_txid, htlc_outputs.clone(), *commitment_number, *their_per_commitment_point, *to_broadcaster_yuv_pixel, logger)
				},
				ChannelMonitorUpdateStep::PaymentPreimage { payment_preimage } => {
					log_trace!(logger, "Updating ChannelMonitor with payment preimage");
					self.provide_payment_preimage(&PaymentHash(Sha256::hash(&payment_preimage.0[..]).to_byte_array()), &payment_preimage, broadcaster, yuv_broadcaster.as_deref(), &bounded_fee_estimator, logger)
				},
				...
				ChannelMonitorUpdateStep::ChannelForceClosed { should_broadcast} => {
					...
					if *should_broadcast {
						...
						self.queue_latest_holder_commitment_txn_for_broadcast(broadcaster, yuv_broadcaster.as_deref(), &bounded_fee_estimator, logger);
		...
		for (claim_id, claim_event) in pending_claim_events {
			match claim_event {
				...
				ClaimEvent::BumpHTLC {
					...
					for htlc in htlcs {
						let htlc_pixel_opt = self.funding_yuv_pixel
							.zip(htlc.htlc.yuv_amount)
							.map(|(funding_pixel, yuv_amount)| {
								Pixel::new(yuv_amount, funding_pixel.chroma)
							});
	...
	fn initial_counterparty_commitment_tx(&mut self) -> Option<CommitmentTransaction> {
		...
		let commitment_tx = self.build_counterparty_commitment_tx(INITIAL_COMMITMENT_NUMBER,
			...
			feerate_per_kw, htlc_outputs, None, None);
		Some(commitment_tx)
	}
	...
	fn build_counterparty_commitment_tx(
		...
		to_broadcaster_yuv_pixel: Option<Pixel>, to_countersignatory_yuv_pixel: Option<Pixel>,
		...
		let keys = TxCreationKeys::from_channel_static_keys(&their_per_commitment_point,
			&broadcaster_keys, &countersignatory_keys, &self.onchain_tx_handler.secp_ctx, None);
	...
	fn sign_to_local_justice_tx(
		...
		let sig = self.onchain_tx_handler.signer.sign_justice_revoked_output(
			&justice_tx, input_idx, value, &per_commitment_key, None, &self.onchain_tx_handler.secp_ctx,
		)?;
	...
	where L::Target: Logger {
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			// otherwise we will lose funds because won't be able to publish justice YUV tx.
			let mut holder_output_idx = 0;
			let mut counterparty_output_idx = None;
			let (counterparty_yuv_pixel, mut output_proofs) = self.funding_yuv_pixel
				.as_ref()
				.map_or((None, None), |funding_yuv_pixel| {
					// The holder's luma that is participated in a specific commitment transaction.
					let counterparty_commitment_luma = self.counterparty_per_commitment_luma
						.as_ref()
						.unwrap()
						.get(&commitment_txid)
						.expect("counterparty commitment luma must be set if funding YUV pixel is set");

					let counterparty_yuv_pixel = if counterparty_commitment_luma.amount != 0 {
						Some(Pixel::new(*counterparty_commitment_luma, funding_yuv_pixel.chroma))
					} else { None };

					(counterparty_yuv_pixel, Some(BTreeMap::new()))
				});

			let mut justice_packages = Vec::new();

			let revokeable_redeemscript = chan_utils::get_revokeable_redeemscript(
				&revocation_pubkey.maybe_tweak(counterparty_yuv_pixel),
			...
			for (idx, outp) in tx.output.iter().enumerate() {
				if outp.script_pubkey == revokeable_p2wsh {
					let mut revk_outp = RevokedOutput::build(
					...
					counterparty_output_idx = Some(idx as u32);
					if idx as u32 == holder_output_idx {
						holder_output_idx += 1;
					}

					if let (Some(yuv_pixel), Some(output_proofs)) = (counterparty_yuv_pixel, output_proofs.as_mut()) {
						let pixel_proof = revk_outp.add_yuv_counterparty_pixel_proof(
							yuv_pixel,
							delayed_key,
							revocation_pubkey,
						);

						output_proofs.insert(idx as u32, pixel_proof);
					}
					...
					let justice_package = PackageTemplate::build_package(
						...
						height,
						counterparty_yuv_pixel,
					);
					justice_packages.push(justice_package);
			...
			let mut htlcs_yuv_amount = 0;
			if let Some(per_commitment_data) = per_commitment_option.cloned().as_mut() {
				// Sort by position in transaction's outputs.
				per_commitment_data.sort_by(|a, b| {
					a.0.transaction_output_index.cmp(&b.0.transaction_output_index)
				});

				for (_, &(ref htlc, _)) in per_commitment_data.iter().enumerate() {
					htlcs_yuv_amount += htlc.yuv_amount.unwrap_or(0);
					if let Some(transaction_output_index) = htlc.transaction_output_index {
						...
						let mut revk_htlc_outp = RevokedHTLCOutput::build(
							per_commitment_point,
							self.counterparty_commitment_params.counterparty_delayed_payment_base_key,
							self.counterparty_commitment_params.counterparty_htlc_base_key,
							per_commitment_key,
							...
							htlc.clone(),
							&self.onchain_tx_handler.channel_transaction_parameters.channel_type_features,
						);

						if transaction_output_index == holder_output_idx ||
							counterparty_output_idx == Some(holder_output_idx) {
							holder_output_idx += 1;
						}

						let yuv_pixel_opt = self.funding_yuv_pixel
							.as_ref()
							.and_then(|funding_yuv_pixel| {
								Some(htlc.get_pixel(funding_yuv_pixel.chroma))
							});

						if let (Some(yuv_pixel), Some(output_proofs)) = (yuv_pixel_opt, output_proofs.as_mut()) {
							let holder_htlc_basepoint = self.onchain_tx_handler.signer.pubkeys().htlc_basepoint;
							let pixel_proof = revk_htlc_outp.add_yuv_htlc_pixel_proof(
								yuv_pixel,
							...
							output_proofs.insert(transaction_output_index, pixel_proof);
						}
			...
			if counterparty_output_idx == Some(holder_output_idx) {
				holder_output_idx += 1;
			}

			if (holder_output_idx as usize) < tx.output.len() && self.funding_yuv_pixel.is_some() {
				let holder_payment_point = self.onchain_tx_handler.signer
					.pubkeys()
					.payment_point;
				...
				let holder_pixel_proof = self.get_holder_yuv_pixel_proof(
					counterparty_yuv_pixel,
					htlcs_yuv_amount,
					holder_payment_point,
				);

				if let Some(pixel_proof) = holder_pixel_proof {
					output_proofs.insert(holder_output_idx, pixel_proof);
				}
			}
			...
			for mut justice_package in justice_packages {
				if let Some(funding_yuv_pixel) = self.funding_yuv_pixel {
					...
					justice_package.set_yuv_commitment_tx(YuvTransaction {
						bitcoin_tx: tx.clone(),
		...
		for (_, &(ref htlc, _)) in htlc_outputs.iter().enumerate() {
			if let Some(transaction_output_index) = htlc.transaction_output_index {
				...
				if preimage.is_some() || !htlc.offered {
					let mut counterparty_htlc_outp = if htlc.offered {
					...
					let htlc_pixel_opt = self.funding_yuv_pixel.map(|funding_pixel| {
						let htlc_pixel = htlc.get_pixel(funding_pixel.chroma);

						let htlc_pixel_proof = get_yuv_htlc_pixel_proof(
							htlc_pixel,
							htlc,
							&per_commitment_point,
							&self.onchain_tx_handler.signer.pubkeys().htlc_basepoint,
							&self.counterparty_commitment_params.counterparty_htlc_base_key,
							&self.holder_revocation_basepoint,
							&self.onchain_tx_handler.secp_ctx,
						);

						counterparty_htlc_outp.set_yuv_pixel_proof(htlc_pixel_proof);
	...
	fn get_broadcasted_holder_claims(&self, holder_tx: &HolderSignedTx, conf_height: u32) -> BroadcastedHolderClaims {
		let mut claim_requests = Vec::with_capacity(holder_tx.htlc_outputs.len());
		let mut htlc_yuv_revokable_scripts = Vec::with_capacity(holder_tx.htlc_outputs.len());
		
		let revocation_key = holder_tx.revocation_key.maybe_tweak(holder_tx.yuv_pixel);
		let redeemscript = chan_utils::get_revokeable_redeemscript(&revocation_key, self.on_holder_tx_csv, &holder_tx.delayed_payment_key);
		...
		for &(ref htlc, _, _) in holder_tx.htlc_outputs.iter() {
			if let Some(transaction_output_index) = htlc.transaction_output_index {
				let mut htlc_output = if htlc.offered {
				...
				let yuv_pixel_opt = self.funding_yuv_pixel.map(|funding_pixel| {
					let htlc_pixel = htlc.get_pixel(funding_pixel.chroma);

					let htlc_revocation_key = holder_tx.revocation_key.tweak(htlc_pixel);
					let htlc_redeemscript = chan_utils::get_revokeable_redeemscript(
						&htlc_revocation_key,
						self.on_holder_tx_csv,
						&holder_tx.delayed_payment_key,
					);
					htlc_yuv_revokable_scripts.push(htlc_redeemscript.to_v0_p2wsh());

					let counterparty_revocation_basepoint = &self.onchain_tx_handler
					...
					htlc_output.set_yuv_pixel_proof(htlc_pixel_proof);
				...
				let htlc_package = PackageTemplate::build_package(
					...
					htlc.cltv_expiry, conf_height, yuv_pixel_opt
				);
				claim_requests.push(htlc_package);
			}
		}

		BroadcastedHolderClaims {
			claim_requests,
			revokable_script: broadcasted_holder_revokable_script,
			htlc_yuv_revokable_scripts,
		}
	}
	...
	fn check_spend_holder_transaction<L: Deref>(&mut self, tx: &Transaction, height: u32, block_hash: &BlockHash, logger: &L) -> Option<(Vec<PackageTemplate>, TransactionOutputs)> where L::Target: Logger {
		...
		macro_rules! append_onchain_update {
			($updates: expr, $to_watch: expr) => {
				claim_requests = $updates.claim_requests;
				self.broadcasted_holder_revokable_script = $updates.revokable_script;
				self.broadcasted_htlc_yuv_revokable_scripts.append(&mut $updates.htlc_yuv_revokable_scripts);
		...
		if self.current_holder_commitment_tx.txid == commitment_txid {
			...
			let mut res = self.get_broadcasted_holder_claims(&self.current_holder_commitment_tx, height);
			...
			if holder_tx.txid == commitment_txid {
				...
				let mut res = self.get_broadcasted_holder_claims(holder_tx, height);
	...
	fn unsafe_get_latest_holder_commitment_txn<L: Deref>(
		...
		for htlc in self.current_holder_commitment_tx.htlc_outputs.iter() {
			if let Some(vout) = htlc.0.transaction_output_index {
				...
				if let Some((htlc_tx, _)) = self.onchain_tx_handler.get_maybe_signed_htlc_tx(
	...
	fn block_connected<B: Deref, YB: Deref, F: Deref, L: Deref>(
			...
			L::Target: Logger,
	{
		...
		let bounded_fee_estimator = LowerBoundedFeeEstimator::new(fee_estimator);
		self.transactions_confirmed(header, txdata, height, broadcaster, yuv_broadcaster, &bounded_fee_estimator, logger)
	}

	fn best_block_updated<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&mut self,
		header: &Header,
		height: u32,
		broadcaster: B,
		...
		L::Target: Logger,
	{
		...
		if height > self.best_block.height {
			...
			self.block_confirmed(height, block_hash, vec![], vec![], vec![], &broadcaster, yuv_broadcaster, &fee_estimator, logger)
			...
			self.onchain_tx_handler.block_disconnected(
				height + 1, broadcaster, yuv_broadcaster.as_deref(), fee_estimator, logger,
			);
			Vec::new()
		} else { Vec::new() }
	}

	fn transactions_confirmed<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&mut self,
		header: &Header,
		txdata: &TransactionData,
		height: u32,
		...
		L::Target: Logger,
	{
		...
		'tx_iter: for tx in &txn_matched {
			...
			if self.funding_yuv_pixel.is_some() {
				self.pending_yuv_payments.entry(txid).or_insert(None);
			}
		...
		self.block_confirmed(height, block_hash, txn_matched, watch_outputs, claimable_outpoints, &broadcaster, yuv_broadcaster, &fee_estimator, logger)
	}
	...
	fn block_confirmed<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&mut self,
		conf_height: u32,
		conf_hash: BlockHash,
		...
		L::Target: Logger,
	{
		...
		for entry in onchain_events_reaching_threshold_conf.drain(..) {
			match entry.event {
				OnchainEvent::HTLCUpdate { ref source, payment_hash, htlc_value_satoshis, htlc_value_yuv, commitment_tx_output_idx } => {
				...
				OnchainEvent::MaturingOutput { descriptor } => {
					...
					// If it is a YUV descriptor, it also needs to wait for the output to be
					// confirmed on L1 YUV Network.
					if let Some(txid) = Self::is_yuv_descriptor(&descriptor) {
						self.pending_yuv_payments.insert(txid, Some(descriptor));
						continue
					}
		...
		self.onchain_tx_handler.update_claims_view_from_requests(claimable_outpoints, conf_height, self.best_block.height, broadcaster, yuv_broadcaster.as_deref(), fee_estimator, logger);
		self.onchain_tx_handler.update_claims_view_from_matched_txn(&txn_matched, conf_height, conf_hash, self.best_block.height, broadcaster, yuv_broadcaster.as_deref(), fee_estimator, logger);
	...
	fn block_disconnected<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&mut self, header: &Header, height: u32, broadcaster: B, yuv_broadcaster: Option<YB>, fee_estimator: F, logger: &WithChannelMonitor<L>
	) where B::Target: BroadcasterInterface,
		YB::Target: YuvBroadcaster,
		F::Target: FeeEstimator,
		L::Target: Logger,
	{
		...
		let bounded_fee_estimator = LowerBoundedFeeEstimator::new(fee_estimator);
		self.onchain_tx_handler.block_disconnected(height, broadcaster, yuv_broadcaster, &bounded_fee_estimator, logger);

		self.best_block = BestBlock::new(header.prev_blockhash, height - 1);
	}

	fn transaction_unconfirmed<B: Deref, YB: Deref, F: Deref, L: Deref>(
		&mut self,
		txid: &Txid,
		broadcaster: B,
		yuv_broadcaster: Option<YB>,
		...
		L::Target: Logger,
	{
		...
		self.onchain_tx_handler.transaction_unconfirmed(txid, broadcaster, yuv_broadcaster, fee_estimator, logger);
	}
	...
	fn is_resolving_htlc_output<L: Deref>(
		...
		'outer_loop: for input in &tx.input {
						...
						for &(ref pending_htlc, ref pending_source) in self.counterparty_claimable_outpoints.get(&txid).unwrap() {
							if pending_htlc.payment_hash == $htlc_output.payment_hash && pending_htlc.amount_msat == $htlc_output.amount_msat {
								if let &Some(ref source) = pending_source {
									...
									payment_data = Some(((**source).clone(), $htlc_output.payment_hash, $htlc_output.amount_msat, $htlc_output.yuv_amount));
									break;
								}
							}
						}
			...
			if let Some((source, payment_hash, amount_msat, yuv_amount)) = payment_data {
	...
	fn get_spendable_outputs(&self, tx: &Transaction) -> Vec<SpendableOutputDescriptor> {
		...
		for (i, outp) in tx.output.iter().enumerate() {
			...
			let yuv_destination_scripts = &self.onchain_tx_handler.yuv_destination_scripts;
			if yuv_destination_scripts.contains(&outp.script_pubkey) {
				spendable_outputs.push(SpendableOutputDescriptor::YuvStaticOutput(YuvStaticOutputDescriptor {
					outpoint: OutPoint::new(tx.txid(), i as u16),
					output: outp.clone(),
					// It will be set after YUV transaction will be confirmed on YUV L1 YUV network.
					yuv_pixel_proof: None,
				}));
			}

			if let Some(ref broadcasted_holder_revokable_script) = self.broadcasted_holder_revokable_script {
				let is_yuv_revokable_script = self.broadcasted_htlc_yuv_revokable_scripts
					.contains(&outp.script_pubkey);

				if is_yuv_revokable_script || broadcasted_holder_revokable_script.0 == outp.script_pubkey {
			...
			if let Some(counterparty_lumas) = self.counterparty_per_commitment_luma.as_ref() {
				...
				let yuv_payment_script = ScriptBuf::new_v0_p2wpkh(
	...
	fn get_holder_yuv_pixel_proof(
		&self,
		counterparty_yuv_pixel: Option<Pixel>,
		htlcs_yuv_amount: u128,
		holder_payment_point: PublicKey,
	) -> Option<PixelProof> {
		let Some(funding_yuv_pixel) = self.funding_yuv_pixel else {
			return None
		};

		let counterparty_yuv_amount = counterparty_yuv_pixel
			.map(|pixel| pixel.luma.amount)
			.unwrap_or(0);

		let holder_yuv_amount = funding_yuv_pixel.luma.amount
			.saturating_sub(counterparty_yuv_amount)
			.saturating_sub(htlcs_yuv_amount);

		if holder_yuv_amount == 0 {
			return None
		}

		let holder_pixel = Pixel::new(holder_yuv_amount, funding_yuv_pixel.chroma);
		let pixel_proof = PixelProof::sig(holder_pixel, holder_payment_point);

		Some(pixel_proof)
	}
}

impl<Signer: WriteableEcdsaChannelSigner, T: Deref, YT: Deref, F: Deref, L: Deref> chain::Listen for (ChannelMonitor<Signer>, T, Option<YT>, F, L)
where
	...
	L::Target: Logger,
{
	fn filtered_block_connected(&self, header: &Header, txdata: &TransactionData, height: u32) {
		self.0.block_connected(header, txdata, height, &*self.1, self.2.as_deref(), &*self.3, &self.4);
	}

	fn block_disconnected(&self, header: &Header, height: u32) {
		self.0.block_disconnected(header, height, &*self.1, self.2.as_deref(), &*self.3, &self.4);
	}
}

impl<Signer: WriteableEcdsaChannelSigner, M, T: Deref, YT: Deref, F: Deref, L: Deref> chain::Confirm for (M, T, Option<YT>, F, L)
where
	...
	L::Target: Logger,
{
	fn transactions_confirmed(&self, header: &Header, txdata: &TransactionData, height: u32) {
		self.0.transactions_confirmed(header, txdata, height, &*self.1, self.2.as_deref(), &*self.3, &self.4);
	}

	fn transaction_unconfirmed(&self, txid: &Txid) {
		self.0.transaction_unconfirmed(txid, &*self.1, self.2.as_deref(), &*self.3, &self.4);
	}

	fn best_block_updated(&self, header: &Header, height: u32) {
		self.0.best_block_updated(header, height, &*self.1, self.2.as_deref(), &*self.3, &self.4);
	}
...
impl<'a, 'b, ES: EntropySource, SP: SignerProvider> ReadableArgs<(&'a ES, &'b SP)>
	...
	fn read<R: io::Read>(reader: &mut R, args: (&'a ES, &'b SP)) -> Result<Self, DecodeError> {
			...
			() => {
				{
					...
					HTLCOutputInCommitment {
						offered, amount_msat, cltv_expiry, payment_hash, transaction_output_index, yuv_amount: None
					}
				}
			}
		}
		...
		let mut pending_yuv_payments_opt: Option<HashMap<Txid, Option<SpendableOutputDescriptor>>> = None;
		let mut counterparty_per_commitment_luma: Option<HashMap<Txid, Luma>> = None;
		// NOTE: Optional YUV amounts for HTLCS in claimable outpoints that are
		// seperatly stored as TLV for back compatability.
		let mut counterparty_claimable_outpoints_yuv_amounts: Option<HashMap<Txid, Vec<Option<u128>>>> = None;
		let mut broadcasted_htlc_yuv_revokable_scripts_opt: Option<Vec<ScriptBuf>> = None;
		read_tlv_fields!(reader, {
			...
			(200, funding_yuv_pixel, option),
			(201, pending_yuv_payments_opt, option),
			(202, counterparty_per_commitment_luma, option),
			(203, counterparty_claimable_outpoints_yuv_amounts, option),
			(204, broadcasted_htlc_yuv_revokable_scripts_opt, optional_vec),
		});
		...
		let pending_yuv_payments = pending_yuv_payments_opt.unwrap_or_else(|| new_hash_map());

		if let Some(yuv_amounts) = counterparty_claimable_outpoints_yuv_amounts {
			for (txid, yuv_amounts) in yuv_amounts {
				let Some(htlcs) = counterparty_claimable_outpoints.get_mut(&txid) else {
					return Err(DecodeError::InvalidValue);
				};

				let mut iter = yuv_amounts.into_iter();
				for (htlc_output, _htlc_source) in htlcs.iter_mut() {
					htlc_output.yuv_amount = iter.next().ok_or_else( || {
		...
		let broadcasted_htlc_yuv_revokable_scripts =
			broadcasted_htlc_yuv_revokable_scripts_opt.unwrap_or_default();
...
mod tests {
	...
	use crate::util::test_utils::{TestLogger, TestBroadcaster, TestFeeEstimator, TestYuvBroadcaster};
	...
	fn do_test_funding_spend_refuses_updates(use_local_txn: bool) {
		...
		let yuv_broadcaster = chanmon_cfgs[1].yuv_tx_broadcaster.as_ref().map(|v| v as &dyn YuvBroadcaster);

		assert!(
			pre_update_monitor.update_monitor(&replay_update, &&broadcaster, yuv_broadcaster, &&chanmon_cfgs[1].fee_estimator, &nodes[1].logger)
			.is_err());
	...
	fn test_prune_preimages() {
		...
		let yuv_broadcaster = Arc::new(TestYuvBroadcaster::new());
		...
		let monitor = ChannelMonitor::new(Secp256k1::new(), keys,
			...
			best_block, dummy_key, None, None, channel_id);
		...
		monitor.provide_latest_counterparty_commitment_tx(Txid::from_byte_array(Sha256::hash(b"1").to_byte_array()),
			preimages_slice_to_htlc_outputs!(preimages[5..15]), 281474976710655, dummy_key, None, &logger);
		monitor.provide_latest_counterparty_commitment_tx(Txid::from_byte_array(Sha256::hash(b"2").to_byte_array()),
			preimages_slice_to_htlc_outputs!(preimages[15..20]), 281474976710654, dummy_key, None, &logger);
		for &(ref preimage, ref hash) in preimages.iter() {
			let bounded_fee_estimator = LowerBoundedFeeEstimator::new(&fee_estimator);
			monitor.provide_payment_preimage(hash, preimage, &broadcaster, Some(yuv_broadcaster.clone()), &bounded_fee_estimator, &logger);
		}
		...
		monitor.provide_latest_counterparty_commitment_tx(Txid::from_byte_array(Sha256::hash(b"3").to_byte_array()),
			preimages_slice_to_htlc_outputs!(preimages[17..20]), 281474976710653, dummy_key, None, &logger);
		...
		monitor.provide_latest_counterparty_commitment_tx(Txid::from_byte_array(Sha256::hash(b"4").to_byte_array()),
			preimages_slice_to_htlc_outputs!(preimages[18..20]), 281474976710652, dummy_key, None, &logger);
