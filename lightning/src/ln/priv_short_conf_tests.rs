...
fn test_scid_privacy_on_pub_channel() {
	...
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 100000, 10001, 42, None, None, Some(scid_privacy_cfg)).unwrap();
...
fn test_inbound_scid_privacy() {
	...
	nodes[1].node.create_channel(nodes[2].node.get_our_node_id(), 100_000, 10_000, 42, None, None, Some(no_announce_cfg)).unwrap();
...
fn test_0conf_channel_with_async_monitor() {
	...
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 100000, 10001, 42, None, None, Some(chan_config)).unwrap();
...
fn test_zero_conf_accept_reject() {
	...
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 100000, 10001, 42,
		None, None, Some(manually_accept_conf)).unwrap();
...
fn test_connect_before_funding() {
	...
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 100_000, 10_001, 42, None, None, None).unwrap();
	let open_channel = get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, nodes[1].node.get_our_node_id());
