...
fn test_monitor_and_persister_update_fail() {
	...
	let yuv_tx_broadcaster = &TestYuvBroadcaster::new() as &dyn YuvBroadcaster;
	let chain_mon = {
		let new_monitor = {
...
fn do_during_funding_monitor_fail(confirm_a_first: bool, restore_b_before_conf: bool, restore_b_before_lock: bool) {
	...
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 100000, 10001, 43, None, None, None).unwrap();
